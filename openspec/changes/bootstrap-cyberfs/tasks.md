...
- [x] 7.9 Implement the async rewrap path for large subtrees. A grant over a subtree with more than `ASYNC_REWRAP_THRESHOLD_NODES` encrypted nodes is created pending and confers no access; a `RewrapJob` (scheduled on `REWRAP_CRON`, registered in `EXPECTED_JOBS`) rewraps every encrypted descendant for the recipient, then activates the grant only after a clean verification pass — crash-safe and idempotent. Pending grants are excluded from recipient-facing permission resolution (`highest_role_over`), the shared-with-me listing (`list_for_subject`), and metadata search, so a partially rewrapped share never appears usable. The create-during-pending gap is closed by the worker re-walking the full subtree at activation (approach b). Below the threshold sharing stays synchronous and immediately usable, unchanged.
